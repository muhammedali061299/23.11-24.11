...
class Ilan(ABC):
    ...
    # def bilgi(self): pass
    @abstractmethod # bir metodun öncesine yazıldığında miras alan alt sınıflarda bu metodun olmasını zorlar.
    def bilgi(self):
        return f"\n\nİlan Bilgisi:\n\
    İlan no:{self.ilanNo}\n\
    Fiyatı :{self.fiyat}\n"
...
class Emlak(Ilan):
    def __init__(self,fiyat="---",no="1",adres="--",tarih="",tip="-",kimden="sahibinden"):
...
class KiralikEv(Emlak):
    ...
    def bilgi(self):
        return f"\n\nKiralık ev ilan Bilgisi:\n\
    İlan no:{self.ilanNo}\n\
    Fiyatı :{self.fiyat}\n\
    Depozitosu: {self.Depozitos}"
...
class SatilikEv(Emlak):
    ...
    def bilgi(self):
        return f"\n\nSatılık ev ilan Bilgisi:\n\
    İlan no:{self.ilanNo}\n\
    Fiyatı :{self.fiyat}\n\
    Tapu durumu: {self.tapuDurumu}"
...
print ("ilan4 ilanNoı: ",ilan4.ilanNo)
