...
class GirisEkrani(QMainWindow):
    ...
    def __init__(self):
        ...
        self.kullanici_adi.textChanged.connect(self.kontrol1) # Line edit içindeki tuşa basma olayı
    ...
    def kontrol1(self):
        kullanicilar = ["qqq","admin","erdinc","Ali"]
        gka = self.kullanici_adi.text()
        # if gka == "qqq": print("Bu kullanıcı adı var.")
        if gka in kullanicilar: print("Bu kullanıcı adı var.")
        else:
            print("Bu kullanıcı sistemde yok.")
...
class AnaEkran(QMainWindow):
    def __init__(self):
        ...
        etiket1 = QLabel("REHBER ANA EKRANI")
        etiket2 = QLabel("REHBER ANA EKRANI")
        etiket2.setStyleSheet("border: 1px double blue; color: red; font-size:22px; background: gray") # CSS kodu ile şekillendirilebilir.
        # mainIcerik.addWidget(etiket1)
        mainIcerik.addWidget(etiket2)
